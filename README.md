...
- Источник истины — БД. Уведомление создаётся в `processing`, доставляется джобой (5 ретраев с backoff), терминальный сбой — `failed`. Зависшие джобы добирает watchdog-крон; переходы статусов — условными UPDATE, гонок нет.
