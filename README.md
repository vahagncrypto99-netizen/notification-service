...
- Домены разделены по сущностям: `app/Domains/Notification` (доставка: статусы, ретраи, события) и `app/Domains/Report` (отчёты: генерация, форматтеры, файлы); Report зависит от Notification только через агрегацию. `app/Services/Delivery` — доставка (именованные mail-сендеры с Reply-To-логикой, Telegram-клиент с троттлингом 30 msg/sec, форматтер сообщений). Связаны через `ChannelSenderInterface`: новый канал — класс + строчка в конфиге.
